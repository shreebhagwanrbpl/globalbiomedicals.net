...
export async function fetchAdminCatalog(customOptions = {}) {
  ...
  const companyId = customOptions.companyId || config.companyId || "global";
  const websiteId = customOptions.websiteId || config.normalizedWebsiteId || "globalbiomedicalsnet";
  ...
  try {
    ...
    for (const item of rawProducts) {
      ...
      if (item.websiteIds && !isItemVisibleOnWebsite(item, config.normalizedWebsiteId)) {
        continue;
      }
      if (
