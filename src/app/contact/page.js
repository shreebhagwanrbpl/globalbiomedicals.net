...
export default function ContactPage({ city: initialCity }) {
  ...
  const initialDistrictName =
    initialCity ||
    (routeDistrict ? formatDistrictName(routeDistrict) : "Jaipur");
  ...
  const handleDistrictSelect = (districtName) => {
    ...
    toast.success(`Updated location for ${districtName} District`);
  };
    ...
    const loadContactData = async () => {
      try {
        ...
        if (Array.isArray(distList) && distList.length > 0) {
          const list = distList.map((d) => ({
            slug: d.slug || d.id,
            name: d.district || d.name || formatDistrictName(d.slug || d.id),
  ...
  return (
    <>
      {/* Banner */}
      <PageBanner
        title={`Contact Us ${selectedDistrict ? `- ${selectedDistrict}` : ""}`}
        subtitle={`Reach out to Global Biomedical for machine inquiries, service contracts, or 24/7 technical help${
          ...
          <div className="container-custom">
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-xs text-[#A08875] font-medium uppercase tracking-wider">
                    Choose District Location
                  </span>
                  <h3 className="text-lg font-bold text-white">
                    Active Support Hub:{" "}
                    <span className="text-[#C49A6C]">{selectedDistrict}</span>
                  </h3>
                </div>
              </div>
                ...
                >
                  {availableDistricts.map((d) => (
                    <option
                      key={d.slug}
                      value={d.name}
                      className="bg-[#2F241E] text-white"
                    >
        ...
        <div className="container-custom grid lg:grid-cols-2 gap-14 items-start">
          ...
          <div>
            ...
            <p className="mt-4 leading-7 text-[#6B5F55] text-base md:text-lg">
              Whether you need a quick quote on blood counters, help setting up
              a new lab room, or emergency machine repair
              {selectedDistrict ? ` in ${selectedDistrict}` : ""}, our team is
              ready to assist.
            </p>
          ...
          <div className="rounded-[40px] border border-[#E6D8C8] bg-[#FFFDF9] p-8 shadow-[0_20px_60px_rgba(111,78,55,0.12)] lg:p-10">
            ...
            <p className="mt-3 text-sm text-[#6B5F55]">
              Fill out the simple form below and our team will get in touch with
              you shortly.
            </p>
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              {availableDistricts.length > 0 && (
                <div>
                  <label className="text-xs font-semibold text-[#2F241E] block mb-1">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <h3 className="text-2xl font-extrabold text-[#2F241E]">
                District Service Location Map:{" "}
                <span className="text-[#8B5A2B]">{selectedDistrict}</span>
              </h3>
            </div>
            <div className="flex items-center gap-2 text-xs font-semibold text-[#6B5F55] bg-[#F5EBDD] px-3.5 py-1.5 rounded-full">
              <Navigation size={14} className="text-[#8B5A2B]" /> GPS Active
              Location
            </div>
          </div>
